...
@Service
public class ApplicationIndustryServiceImpl implements ApplicationIndustryService {
    ...
    @Override
    @Transactional
    public R submitIndustryApply(String userId, IndustryApplySubmitDTO form) {
        try {
            // 1. 查询项目信息并校验申请时间
            ...
            // 5. 校验员工所在企业是否已提交企业申请并通过审批
            String companyId = applicant.getCompanyId();
            if (companyId == null || companyId.isEmpty()) {
                return R.error("申请人未关联企业信息");
            }
            ...
            // 校验企业申请状态是否为4（企业审核通过）
            if (enterpriseApplication.getApplyStatus() == null || enterpriseApplication.getApplyStatus() != 4) {
                return R.error("该企业的项目申请尚未通过审核，无法提交个人申请");
            }
            ...
            // 7. 创建申请记录
            ApplicationIndustryTalent industryApply = new ApplicationIndustryTalent();
            
            // 生成申请ID
            String applicationId = UUID.randomUUID().toString().replace("-", "");
            industryApply.setApplicationId(applicationId);
            ...
            industryApply.setApplySort(0);
            industryApply.setCurrentSort(0);
            ...
            return R.success("产业人才申请提交成功");
            
        } catch (Exception e) {
            return R.error("产业人才申请提交失败：" + e.getMessage());
        }
    }
    ...
    private boolean hasPendingApplication(String userId, String projectId) {
        ...
        // 排除已拒绝的状态：4-工作单位审核拒绝、7-住建委审核拒绝、10-住建委审核拒绝、13-组织部审核拒绝
        wrapper.notIn("apply_status", 4, 7, 10, 13);
        
        int count = applicationIndustryTalentMapper.selectCount(wrapper);
        return count > 0;
    }
    ...
    @Override
    @Transactional
    public R withdrawIndustryApply(String userId, String applicationId) {
        try {
            // 1. 查询申请记录
            ...
            // 2. 校验申请是否为该用户提交
            if (!userId.equals(industryApply.getApplicantId())) {
            ...
            industryApply.setApplyStatus(0);
    ...
    @Override
    @Transactional
    public R updateIndustryApply(String userId, IndustryApplyUpdateDTO form) {
        try {
            ...
            // 3. 校验申请状态是否为撤回/待提交（0）
            if (industryApply.getApplyStatus() == null || industryApply.getApplyStatus() != 0) {
            ...
            return R.error("产业人才申请修改失败：" + e.getMessage());
        }
    }
    ...
    @Override
    public R queryIndustryApplications(String userId) {
        try {
            // 1. 校验用户是否存在
